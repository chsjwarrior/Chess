...
namespace bitBoardOperations {
	...
	void setInitialPosition(BitBoard& bitBoard) {
		for (uint8_t namePiece = 0; namePiece < 6; namePiece++) {
			bitBoard.bitMaps[namePiece][pieces::WHITE] = INITIAL_POSITION[namePiece][pieces::WHITE];
			bitBoard.bitMaps[namePiece][pieces::BLACK] = INITIAL_POSITION[namePiece][pieces::BLACK];
		}
		bitBoard.flags = bitBoard.bitMaps[pieces::KING][pieces::WHITE] | bitBoard.bitMaps[pieces::KING][pieces::BLACK] | bitBoard.bitMaps[pieces::ROOK][pieces::WHITE]
			| bitBoard.bitMaps[pieces::ROOK][pieces::BLACK];
		bitBoard.attacks = 0;
	}
	...
	std::optional<Piece> getPieceFromSquare(const BitBoard& bitBoard, const uint8_t& square) {
		...
		for (pieces::NAME namePiece : pieces::allPiecesName)
			if (hasIntersection(bitBoard.bitMaps[namePiece][pieces::BLACK], bitmap))
				return std::make_optional<Piece>(namePiece, pieces::BLACK);
			else if (hasIntersection(bitBoard.bitMaps[namePiece][pieces::WHITE], bitmap))
				return std::make_optional<Piece>(namePiece, pieces::WHITE);
	...
	const bool isSquareOccupied(const BitBoard& bitBoard, const pieces::COLOR& color, const uint8_t& square) {
		uint64_t bitmap = getBitmapFromSquare(square);
	...
	const bool isKingCheck(const BitBoard& bitBoard, const uint8_t& colorKing) {
		return bitBoardOperations::hasIntersection(bitBoard.attacks, bitBoard.bitMaps[pieces::KING][colorKing]);
	}
};